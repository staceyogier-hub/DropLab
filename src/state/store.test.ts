...
describe('store — load → analyse flow', () => {
  ...
  it('applies a preset, generates and computes a result', async () => {
    const s = useStore.getState();
    await s.applyPreset('a22-lv');
  ...
  it('switches to external-lift analysis for an EL preset', async () => {
    await useStore.getState().applyPreset('el-medium');
  ...
  it('clamps the CFC to one valid for the sample rate', async () => {
    await useStore.getState().applyPreset('a22-lv');
    await useStore.getState().generateFromConfig();
    // Generator default fs = 2000 Hz; CFC600 is invalid → clamped down.
    await useStore.getState().setCfc(600);
    expect(useStore.getState().cfc).not.toBe(600);
  });

  it('recomputing with a lower CFC does not increase the peak', async () => {
    await useStore.getState().applyPreset('a22-hv');
    await useStore.getState().generateFromConfig();
    await useStore.getState().setCfc(180);
    const hi = useStore.getState().result!.summary.peakImpactAccelG;
    await useStore.getState().setCfc(60);
